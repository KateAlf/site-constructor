...
function init() {
  ...
  addButtonElements.forEach((item) => item.addEventListener(`click`, showAddMenuHandler));

  const chooseButtonElements = document.querySelectorAll(`.choose-elem__btn`);
  chooseButtonElements.forEach((item) => item.addEventListener(`click`, addElementHandler));
...
function buttonDeleteHandler(evt) {
  ...
  const blockIndex = currentData.findIndex((item) => item.id === id);
